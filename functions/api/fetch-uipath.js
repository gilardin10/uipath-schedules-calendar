...
export async function onRequestPost({ request }) {
  ...
  if (action === 'getToken') {
    ...
    if (body.tenant) params.acr_values = `tenant:${body.tenant}`;
  ...
  // ── 4. On-prem PKCE: authorization-code → token exchange (custom token URL) ──
  if (action === 'onPremPkceExchange') {
    const { code, verifier, clientId, redirectUri, tokenUrl } = body;
    if (!code || !verifier || !clientId || !redirectUri || !tokenUrl)
      return json({ ok: false, error: 'onPremPkceExchange requires code, verifier, clientId, redirectUri, tokenUrl' }, 400);
    ...
    try {
      ...
      return json({ ok: false, error: `Network error reaching on-prem identity server: ${err.message}` }, 502);
    }
    if (!tokenRes.ok) {
      let detail = '';
      try { const t = await tokenRes.text(); detail = t ? ` — ${t.slice(0, 300)}` : ''; } catch (_) {}
      return json({ ok: false, error: `Token endpoint returned HTTP ${tokenRes.status}${detail}` }, 502);
    }
    let td;
    try { td = await tokenRes.json(); }
    catch { return json({ ok: false, error: 'Identity server returned a non-JSON response' }, 502); }
  ...
  // ── 5. All remaining actions require a valid Authorization header ─────────
  ...
  if (action === 'schedules') {
    ...
    if (!/^[\w\s\-\.@:()]+$/.test(releaseName))
      return json({ ok: false, error: 'Invalid release name format' }, 400);
